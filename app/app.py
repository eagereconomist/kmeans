...
raw_label = uploaded.name
raw_df = pd.read_csv(uploaded)

# if the upload changed, reset our working copy & did_cluster
if (
    "df_source_label" not in st.session_state
    or st.session_state.df_source_label != raw_label
):
    st.session_state.df = raw_df.copy()
    st.session_state.df_source_label = raw_label
    st.session_state.did_cluster = False

# use the session copy from here on out
...
if initial_clusters:
    ...
    if run_cluster:
        ...
        # update our session_state copy
        st.session_state.df = clustered
        df = st.session_state.df
        st.session_state.did_cluster = True

        cluster_col = f"cluster_{n_clusters}"
        # shift 0→1 & str for categorical legend
        df[cluster_col] = (df[cluster_col].astype(int) + 1).astype(str)
...
if plot_dim == "3D":
    pc_z = st.sidebar.selectbox(
        ...
        [p for p in pcs_in_file if p not in (pc_x, pc_y)],
        index=0,
    )
